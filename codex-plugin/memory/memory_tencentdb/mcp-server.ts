/**
 * TDAI Memory MCP Server — Codex plugin entry point.
 *
 ...
 *
 * Compiled to mcp-server.mjs by `npm run build:codex-plugin`.
 * Launched by Codex via .mcp.json config.
 */
...
async function ensureGateway(): Promise<void> {
  ...
  if (!(await waitForHealth(GATEWAY))) {
    logger.warn(
      "Gateway did not become healthy within 30 s. Search tools will be unavailable.",
    );
  } else {
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: ((await r.json()) as { results?: string }).results ?? "",
          },
        ],
      };
    } catch {
