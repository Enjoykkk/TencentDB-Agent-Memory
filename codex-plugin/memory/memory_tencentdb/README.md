...
#    Copies the plugin to ~/.agents/plugins/, builds self-contained .mjs files,
#    creates marketplace entry, and installs via Codex CLI if available.
node scripts/install-codex-plugin.js

# 2. If Codex CLI is not available on your PATH, install manually:
#    codex plugin add tencentdb-memory@local-plugins

# 3. Restart Codex

# 4. In Settings → Hooks, review and trust both hooks:
#    - UserPromptSubmit (Recalling memory context…)
#    - Stop (Capturing conversation turn…)
```
